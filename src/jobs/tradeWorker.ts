...
export const startTradeWorker = () => {
    ...
    worker = new Worker(
      ...
      async (job: Job) => {
        ...
        console.log(`Processing trade job for tx ${transactionHash} (Job ID: ${job.id})`);
        ...
        try {
            ...
            if (makerAssetId && makerAssetId !== '0') {
                ...
                console.log(`Skipping trade with no valid token ID (maker: ${makerAssetId}, taker: ${takerAssetId})`);
                return;
            }
            ...
            console.log(`Trade details: ${side} ${sharesDisplay} shares of ${tokenId} for $${usdcDisplay} ($${price.toFixed(4)})`);
            ...
            if (market) {
                ...
                console.log(`Matched Market: ${marketSlug}, Outcome: ${outcome}, Liquidity: ${liquidityNum}`);
            } else {
